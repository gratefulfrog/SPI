...
def isNullReturn(responseLis):
    return all(map(lambda x,y: x==y,responseLis,nullResponse))
...
## temp value until init call!
bid = [-1]

def doOneCom(type,spi,q,printResults=False):
    global bid
...
def go(typeLis,q):
    ...
    try:
        ...
        doOneCom(typeLis[0],spi,q,False)
        doSynch()
        #input()
        count = 1
        while True:
            if (count%pollDisplayIterations == 0):
                print(count, ': Polling :',type2NameDict[typeLis[1]])
            count +=1                
