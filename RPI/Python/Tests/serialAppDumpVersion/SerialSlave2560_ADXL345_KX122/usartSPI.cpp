...
USARTSPI::USARTSPI(uint8_t usartID) : YSPI(), uID(usartID) {
  ...
  if (uID == 3){  // for the ADXL345, I hope!
    //usartRegVec[uID].ucsrC |= (1<<UCPHA3)|(1<<UCPOL3);  // but UCPHA3 and UCPOL3 are not defined anywhere I could find, so I improvised...
    usartRegVec[uID].ucsrC |= (1<<1)|(1<<0);   // this seems to work!
  }
